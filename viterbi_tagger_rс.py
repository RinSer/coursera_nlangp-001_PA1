...
fileDev = open('C:\Python2\gene.dev', 'r')
fileE = open('C:\Python2\emissionRC.parameters', 'r')
...
def viterbi(sentence, tags, start_p, trans_p, emit_p):
    ...
    for y in tags:
        if sentence[0] in emit_p:
            ...
            digits = any([c.isdigit() for c in sentence[0]])
            upper = any([c.isupper() for c in sentence[0]])
            if digits:
                V[0]['* ' + y] = start_p['* ' + y] * emit_p['_NUM_'][y]
                path[y] = [y]
            elif all([c.isupper() for c in sentence[0]]):
                V[0]['* ' + y] = start_p['* ' + y] * emit_p['_CAP_'][y]
                path[y] = [y]
            elif sentence[0][-1].isupper():
...
def execute():
    fileOT = open('C:\Python2\gene.vctagged', 'w')
