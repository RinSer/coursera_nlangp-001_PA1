...
def main(mode, count_file, sentence_file):
    if mode not in ["TAGCLASS", "CLASS"]: hmm = HMM(open('C:\Python2\gene.counts', 'r'))
    else: hmm = ClassedHMM(open('C:\Python2\gene.counts', 'r'))                                                
    
    # Run on each sentence.
    for sentence in read_sentences(open('C:\Python2\gene.dev', 'r')):
