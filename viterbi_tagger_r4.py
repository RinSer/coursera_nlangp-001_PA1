...
fileE = open('C:\Python2\emissionR4.parameters', 'r')
...
def viterbi(sentence, tags, start_p, trans_p, emit_p):
    ...
    for y in tags:
        if sentence[0] in emit_p:
            ...
            if sentence[0].isalnum() and not sentence[0].isalpha():
                V[0]['* ' + y] = start_p['* ' + y] * emit_p['_NUM_'][y]
                path[y] = [y]
            elif sentence[0].isupper() and sentence[0].isalpha():
                V[0]['* ' + y] = start_p['* ' + y] * emit_p['_CAP_'][y]
                path[y] = [y]
            elif sentence[0][-1].isupper() and not sentence[0][:-1].isupper():
                V[0]['* ' + y] = start_p['* ' + y] * emit_p['_LCAP_'][y]
                path[y] = [y]
            else:
    ...
    for n in range(1, len(sentence)):
        ...
        for y in tags:
            if sentence[n] in emit_p:
                ...
                if n == 1:
                    if sentence[1].isalnum() and not sentence[1].isalpha():
                        (prob, tag) = max((V[0]['* ' + path[y0][0]] * trans_p[y]['* ' + path[y0][0]] * emit_p['_NUM_'][y], y0) for y0 in tags)
                        V[1][path[tag][0] + ' ' + y] = prob
                        newpath[y] = path[tag] + [y]
                    elif sentence[1].isupper() and sentence[1].isalpha():
                        (prob, tag) = max((V[0]['* ' + path[y0][0]] * trans_p[y]['* ' + path[y0][0]] * emit_p['_CAP_'][y], y0) for y0 in tags)
                        V[1][path[tag][0] + ' ' + y] = prob
                        newpath[y] = path[tag] + [y]
                    elif sentence[1][-1].isupper() and not sentence[1][:-1].isupper():
                        (prob, tag) = max((V[0]['* ' + path[y0][0]] * trans_p[y]['* ' + path[y0][0]] * emit_p['_LCAP_'][y], y0) for y0 in tags)
                        V[1][path[tag][0] + ' ' + y] = prob
                        newpath[y] = path[tag] + [y]
                    else:
                    ...
                    if sentence[n].isalnum() and not sentence[n].isalpha():
                        (prob, tag) = max((V[n-1][str(path[y0][n-2]) + ' ' + str(path[y0][n-1])] * trans_p[y][str(path[y0][n-2]) + ' ' + str(path[y0][n-1])] * emit_p['_NUM_'][y], y0) for y0 in path)
                        V[n][path[tag][n-1] + ' ' + y] = prob
                        newpath[y] = path[tag] + [y]
                    elif sentence[n].isupper() and sentence[n].isalpha():
                        (prob, tag) = max((V[n-1][str(path[y0][n-2]) + ' ' + str(path[y0][n-1])] * trans_p[y][str(path[y0][n-2]) + ' ' + str(path[y0][n-1])] * emit_p['_CAP_'][y], y0) for y0 in path)
                        V[n][path[tag][n-1] + ' ' + y] = prob
                        newpath[y] = path[tag] + [y]
                    elif sentence[n][-1].isupper() and not sentence[n][:-1].isupper():
                        (prob, tag) = max((V[n-1][str(path[y0][n-2]) + ' ' + str(path[y0][n-1])] * trans_p[y][str(path[y0][n-2]) + ' ' + str(path[y0][n-1])] * emit_p['_LCAP_'][y], y0) for y0 in path)
                        V[n][path[tag][n-1] + ' ' + y] = prob
                        newpath[y] = path[tag] + [y]
                    else:
...
def execute():
    fileOT = open('C:\Python2\gene_test.p3.out', 'w')
